...
import { createSolarEngine } from './solar.js';
...
const solarReadout = document.querySelector('#solar-readout');
const solarStateEl = document.querySelector('#solar-state');
const solarTimeEl = document.querySelector('#solar-time');
const solarAltitudeEl = document.querySelector('#solar-altitude');
const solarAzimuthEl = document.querySelector('#solar-azimuth');
const solarLatEl = document.querySelector('#solar-lat');
const solarLonEl = document.querySelector('#solar-lon');
const solarLocateBtn = document.querySelector('#solar-locate');
...
// Real-time astronomical sun: GPS + clock drive the scene light, atmosphere and water highlights.
const solarEngine = createSolarEngine({ scene, renderer, root });
...
initRainfallEditor();
solarLocateBtn?.addEventListener('click', () => solarEngine.locate());
if (ENABLE_TUNING_CONSOLE) initTuningPanelV2();
root.dataset.releaseChannel = ENABLE_TUNING_CONSOLE ? 'development' : 'public';
solarEngine.start();
animate();

function updateSolarUniforms() {
  const sun = solarEngine.state;
  if (solarTimeEl) solarTimeEl.textContent = new Date().toLocaleTimeString('zh-CN', { hour12: false });
  if (solarAltitudeEl) solarAltitudeEl.textContent = `${sun.altitudeDeg.toFixed(1)}°`;
  if (solarAzimuthEl) solarAzimuthEl.textContent = `${sun.azimuthDeg.toFixed(1)}°`;
  if (solarLatEl) solarLatEl.textContent = `${sun.latitude.toFixed(3)}°`;
  if (solarLonEl) solarLonEl.textContent = `${sun.longitude.toFixed(3)}°`;
  if (solarStateEl) solarStateEl.textContent = sun.altitudeDeg > -0.833 ? 'SUN TRACKING' : 'SUN BELOW HORIZON';
  waterPlane.material.uniforms.uSolarDir.value.copy(sun.direction);
  waterPlane.material.uniforms.uSolarColor.value.copy(sun.color);
  waterPlane.material.uniforms.uSolarIntensity.value = sun.intensity;
  waterPlane.material.uniforms.uSolarElevation.value = sun.altitudeRad;

  // Rainform's rain materials are custom shaders, so Three.js lights do not
  // automatically affect them. Feed the real sun vector into their uniforms.
  worldGroup.traverse((node) => {
    const material = node.material;
    if (!material?.uniforms) return;
    if (material.uniforms.uSolarDir) material.uniforms.uSolarDir.value.copy(sun.direction);
    if (material.uniforms.uSolarColor) material.uniforms.uSolarColor.value.copy(sun.color);
    if (material.uniforms.uSolarIntensity) material.uniforms.uSolarIntensity.value = sun.intensity;
    if (material.uniforms.uSolarElevation) material.uniforms.uSolarElevation.value = sun.altitudeRad;
  });
}
...
function createRainChainSystem(baseCount, ambientCount = 0, downpourCount = 0) {
  ...
  const pearlMaterial = new THREE.ShaderMaterial({
    uniforms: {
      ...
      uSolarDir: { value: new THREE.Vector3(0, 1, 0) },
      uSolarColor: { value: new THREE.Color(1, 0.85, 0.58) },
      uSolarIntensity: { value: 0 },
      uSolarElevation: { value: 0 },
...
function createWaterPlane() {
  ...
  const material = new THREE.ShaderMaterial({
    uniforms: {
      ...
      uSolarDir: { value: new THREE.Vector3(0.0, 1.0, 0.0) },
      uSolarColor: { value: new THREE.Color(1, 0.85, 0.58) },
      uSolarIntensity: { value: 0.0 },
      uSolarElevation: { value: 0.0 },
