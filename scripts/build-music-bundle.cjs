...
/**
 * Build script: concatenates all Mineradio JS modules into a single bundle.js
 * This eliminates 102 sequential synchronous XHR requests, reducing load time from ~3 minutes to <100ms.
 *
 * Usage: node scripts/build-music-bundle.cjs
 */
...
// Directories to build bundles for
const TARGETS = [
  path.join(ROOT, 'public', 'music'),
  path.join(ROOT, 'Mineradio-Standalone'),
];

// Module paths (relative to each target dir), same order as index-loader.js
const modulePaths = [
...
console.log('Building Mineradio bundle...');
...
for (const targetDir of TARGETS) {
  ...
  for (const relPath of modulePaths) {
    ...
    if (!fs.existsSync(absPath)) {
      console.error('  [' + path.basename(targetDir) + '] Missing: ' + relPath);
      missing = true;
      break;
    }
  ...
  if (missing) {
    console.error('  Skipped: ' + targetDir);
    continue;
  }
  ...
  // Write bundle.js
  const outputPath = path.join(targetDir, 'js', 'bundle.js');
  fs.writeFileSync(outputPath, bundle, 'utf8');

  console.log('  [' + path.basename(targetDir) + '] ' + modulePaths.length + ' modules, ' + (totalSize / 1024).toFixed(1) + ' KB -> ' + outputPath);
}
