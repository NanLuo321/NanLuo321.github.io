...
function soundframechange(t) {
  ...
  localStorage.setItem('w10_volume', v);
  ...
  document.querySelector('.sound-frame .sound-range .sound-data').innerText=v;
}
...
function applyDisplay() {
  // 无论分辨率如何，整个网页始终拉伸铺满窗口
  var scale = parseInt(localStorage.getItem('w10_scale')) || 100;
  var factor = scale / 100;
  factor = Math.max(0.05, Math.min(factor, 2));
  // 缩放容器：桌面 + 任务栏一起缩放
  var ws = document.getElementById('w10-workspace');
  if (!ws) {
    ws = document.createElement('div');
    ws.id = 'w10-workspace';
    ws.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:100%;transform-origin:top left;';
    var desk = document.querySelector('.desk');
    var bar = document.querySelector('.bar');
    if (desk && bar) {
      var qf = document.querySelector('.quick-frames');
      document.body.insertBefore(ws, qf || document.body.firstChild);
      ws.appendChild(desk);
      ws.appendChild(bar);
    }
  }
  ws.style.transform = 'translate(0px,0px) scale(' + factor + ')';
}
window.applyDisplay = applyDisplay;

// ===== 分辨率更改：黑屏 1 秒 + 确认弹窗 =====
function requestResolution(resValue) {
  var prev = localStorage.getItem('w10_resolution');
  localStorage.setItem('w10_resolution', resValue);
  var ov = document.getElementById('w10-res-apply');
  if (!ov) {
    ov = document.createElement('div');
    ov.id = 'w10-res-apply';
    ov.style.cssText = 'position:fixed;left:0;top:0;width:100%;height:100%;background:#000;z-index:99999998;';
...
function clearResConfirm() {
  clearTimeout(window.__resTimer);
  var box = document.getElementById('w10-res-confirm');
  if (box) box.style.display = 'none';
}
function revertResolution() {
  if (window.__resPrev) {
    localStorage.setItem('w10_resolution', window.__resPrev);
    applyDisplay();
  }
  clearResConfirm();
}
function showResConfirm(prev) {
  window.__resPrev = prev;
  var box = document.getElementById('w10-res-confirm');
  if (!box) {
    box = document.createElement('div');
    box.id = 'w10-res-confirm';
    box.innerHTML =
      '<div class="rc-title">是否保留这些显示设置？</div>' +
      '<div class="rc-sub">你的屏幕即将恢复原来的显示设置，除非你选择保留。</div>' +
      '<div class="rc-count" id="rcCount"></div>' +
      '<div class="rc-btns">' +
      '<button id="rcKeep">保留更改</button>' +
      '<button id="rcRevert">恢复原来的显示</button>' +
      '</div>';
    document.body.appendChild(box);
    box.querySelector('#rcKeep').onclick = function (e) { e.stopPropagation(); clearResConfirm(); };
    box.querySelector('#rcRevert').onclick = function (e) { e.stopPropagation(); revertResolution(); };
    box.onclick = function (e) { e.stopPropagation(); };
  }
  box.style.display = 'flex';
  clearTimeout(window.__resTimer);
  var left = 15;
  var cnt = document.getElementById('rcCount');
  function tick() {
    if (!document.getElementById('w10-res-confirm') || document.getElementById('w10-res-confirm').style.display == 'none') return;
    if (left <= 0) { revertResolution(); return; }
    cnt.innerText = '在 ' + left + ' 秒后恢复原来的显示';
    left--;
    window.__resTimer = setTimeout(tick, 1000);
  }
  tick();
}
window.showResConfirm = showResConfirm;

function applySettings() {
  // 主题
  var theme = localStorage.getItem('w10_theme');
  var desk = document.querySelector('.desk');
  var ov = document.getElementById('w10-theme-overlay');
  if (theme == 'dark') {
    if (!ov && desk) {
      ov = document.createElement('div');
    ...
    if (ov) ov.style.cssText = 'position:absolute;left:0;top:0;right:0;bottom:0;background:#0007;pointer-events:none;';
  } else if (ov) {
    ov.remove();
  }
  // 背景：自定义壁纸 > 背景颜色
  var wall = localStorage.getItem('w10_wallpaper');
  if (wall) {
  ...
  // 夜间模式
  var night = localStorage.getItem('w10_nightmode');
  var nov = document.getElementById('w10-night-overlay');
  if (night == '1') {
    if (!nov && desk) {
      nov = document.createElement('div');
      nov.id = 'w10-night-overlay';
      desk.append(nov);
    }
    if (nov) nov.style.cssText = 'position:absolute;left:0;top:0;right:0;bottom:0;background:rgba(255,160,60,.12);pointer-events:none;';
  } else if (nov) {
    nov.remove();
  }
  // 分辨率/缩放
  applyDisplay();
  // 音量同步
  var vol = parseInt(localStorage.getItem('w10_volume'));
  if (isNaN(vol)) vol = 70;
  var frame = document.querySelector('.sound-frame .win-range');
  if (frame) {
    frame.setAttribute('data-value', vol);
    var r2 = frame.querySelector('span.r2');
    var r3 = frame.querySelector('span.r3');
    if (r2) r2.style.width = 'calc((100% - 8px) * ' + (vol / 100) + ')';
    if (r3) r3.style.left = 'calc((100% - 8px) * ' + (vol / 100) + ')';
    var sd = document.querySelector('.sound-frame .sound-data');
    if (sd) sd.innerText = vol;
    var img = document.querySelector('.bar .right .sound img');
    if (img) img.src = '../../img/icon/ui/audio' + (vol == 0 ? 0 : vol <= 33 ? 1 : vol <= 66 ? 2 : 3) + '.png';
  }
}
applySettings();
