...
declare global {
	interface Window {
		...
		scrollBlurHandler?: () => void;
	}
}

// ====== Scroll Blur (滚动模糊) ======
// 在 overlay 模式下，壁纸图片随滚动逐渐模糊：顶部清晰，向下滚动时模糊加深
const SCROLL_BLUR_THRESHOLD = 400; // 滚动到此距离时达到最大模糊

let scrollBlurEnabled = false;

export function getDefaultScrollBlurEnabled(): boolean {
	return false;
}

export function getStoredScrollBlurEnabled(): boolean {
	if (
		typeof localStorage === "undefined" ||
		typeof localStorage.getItem !== "function"
	) {
		return getDefaultScrollBlurEnabled();
	}
	const stored = localStorage.getItem("scrollBlurEnabled");
	if (stored === null) {
		return getDefaultScrollBlurEnabled();
	}
	return stored === "true";
...
export function setScrollBlurEnabled(enabled: boolean): void {
	if (
		typeof localStorage === "undefined" ||
	...
	localStorage.setItem("scrollBlurEnabled", String(enabled));
	applyScrollBlurToDocument(enabled);
}

export function applyScrollBlurToDocument(enabled: boolean): void {
...
function startScrollBlurListener(): void {
	...
	window.scrollBlurHandler = handler;
	window.addEventListener("scroll", handler, { passive: true });
	// 立即执行一次，设置初始状态
	handler();
}

function stopScrollBlurListener(): void {
	if (typeof window === "undefined" || !window.scrollBlurHandler) return;
	window.removeEventListener("scroll", window.scrollBlurHandler);
	window.scrollBlurHandler = undefined;
}
...
function showBannerMode(animate = false) {
	...
	// 移除透明效果（横幅模式不使用半透明）——但若用户启用了横幅透明，则保留
	const bannerTransparency = getStoredBannerTransparencyEnabled();
	adjustMainContentTransparency(bannerTransparency);
...
function showOverlayMode() {
	...
	// 如果启用了滚动模糊，启动监听器（通过 applyScrollBlurToDocument 同步模块状态）
	applyScrollBlurToDocument(getStoredScrollBlurEnabled());
}
...
// Banner transparency functions
// 在横幅壁纸模式下启用半透明卡片效果（融合 overlay 的透明质感）
export function getDefaultBannerTransparencyEnabled(): boolean {
	return false;
}

export function getStoredBannerTransparencyEnabled(): boolean {
	if (
		typeof localStorage === "undefined" ||
		typeof localStorage.getItem !== "function"
	) {
		return getDefaultBannerTransparencyEnabled();
	}
	const stored = localStorage.getItem("bannerTransparencyEnabled");
	if (stored === null) {
		return getDefaultBannerTransparencyEnabled();
	}
	return stored === "true";
...
export function setBannerTransparencyEnabled(enabled: boolean): void {
	if (
		typeof localStorage === "undefined" ||
		typeof localStorage.setItem !== "function"
	) {
	...
	localStorage.setItem("bannerTransparencyEnabled", String(enabled));
	applyBannerTransparencyToDocument(enabled);
}

export function applyBannerTransparencyToDocument(enabled: boolean): void {
	if (typeof document === "undefined") {
		return;
	}
	document.documentElement.setAttribute(
		"data-banner-transparency",
		String(enabled),
	);
	// 仅在当前为 banner 模式时才实际切换透明效果
	const currentMode =
		(document.documentElement.getAttribute(
			"data-wallpaper-mode",
		) as WALLPAPER_MODE) || backgroundWallpaper.mode;
