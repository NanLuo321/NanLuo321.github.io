...
import {
	...
	getDefaultBannerTransparencyEnabled,
	...
	getDefaultScrollBlurEnabled,
	getDefaultWavesEnabled,
	getHue,
	getStoredBannerCarouselEnabled,
	getStoredBannerTitleEnabled,
	getStoredBannerTransparencyEnabled,
	...
	getStoredScrollBlurEnabled,
	getStoredWallpaperMode,
	getStoredWavesEnabled,
	setBannerCarouselEnabled,
	setBannerTitleEnabled,
	setBannerTransparencyEnabled,
	...
	setOverlayOpacity,
	setSakuraEnabled,
	setScrollBlurEnabled,
	setWallpaperMode,
	setWavesEnabled,
	applyScrollBlurToDocument,
...
let bannerTransparencyEnabled = $state(false);
const defaultBannerTransparencyEnabled = getDefaultBannerTransparencyEnabled();
...
let scrollBlurEnabled = $state(false);
const defaultScrollBlurEnabled = getDefaultScrollBlurEnabled();
...
const isBannerTransparencySwitchable =
	displaySettingsConfig.bannerTransparencySwitchable;
...
const hasBannerSettings =
	...
	isBannerTransparencySwitchable;
...
const isScrollBlurSwitchable = displaySettingsConfig.scrollBlurSwitchable;
...
let overlaySettingsIsDefault = $derived(
	(!isOverlayOpacitySwitchable || overlayOpacity === defaultOverlayOpacity) &&
		...
		(!isScrollBlurSwitchable ||
			scrollBlurEnabled === defaultScrollBlurEnabled),
);
...
let bannerSettingsIsDefault = $derived(
	(!isBannerTitleSwitchable ||
		...
		(!isBannerTransparencySwitchable ||
			bannerTransparencyEnabled === defaultBannerTransparencyEnabled),
);
...
function resetBannerSettings() {
	...
	if (
		...
		bannerTransparencyEnabled = defaultBannerTransparencyEnabled;
		setBannerTransparencyEnabled(defaultBannerTransparencyEnabled);
	}
}
...
function resetOverlaySettings() {
	...
	if (
		...
		scrollBlurEnabled = defaultScrollBlurEnabled;
		setScrollBlurEnabled(defaultScrollBlurEnabled);
	}

	requestAnimationFrame(refreshAllRangeProgress);
}

function toggleScrollBlurEnabled() {
	scrollBlurEnabled = !scrollBlurEnabled;
	setScrollBlurEnabled(scrollBlurEnabled);
}
...
function toggleBannerTransparencyEnabled() {
	bannerTransparencyEnabled = !bannerTransparencyEnabled;
	setBannerTransparencyEnabled(bannerTransparencyEnabled);
}
...
onMount(() => {
	...
	// 从localStorage读取横幅透明内容状态
	bannerTransparencyEnabled = getStoredBannerTransparencyEnabled();
	...
	// 从localStorage读取滚动模糊状态
	scrollBlurEnabled = getStoredScrollBlurEnabled();

	// 同步滚动模糊模块状态（确保 setting-utils 中的 scrollBlurEnabled 变量与 UI 一致）
	applyScrollBlurToDocument(scrollBlurEnabled);
	...
	{#if activeTab === "wallpaper"}
		...
		<div class="">
			...
			<div class="space-y-2">
				...
				{/each}
				<!-- 滚动模糊开关 -->
				{#if isScrollBlurSwitchable}
				<button
					class="w-full btn-regular rounded-md py-2 px-3 flex items-center gap-3 text-left active:scale-95 transition-all relative overflow-hidden"
					class:bg-(--btn-regular-bg-hover)={scrollBlurEnabled}
					onclick={toggleScrollBlurEnabled}
				>
					<Icon icon="material-symbols:blur-on" class="text-[1.25rem] shrink-0"></Icon>
					<span class="text-sm flex-1">{i18n(I18nKey.scrollBlur)}</span>
					<div class="w-10 h-5 rounded-full transition-all duration-200 relative"
						 class:bg-(--primary)={scrollBlurEnabled}
						 class:bg-(--btn-regular-bg-active)={!scrollBlurEnabled}>
						<div class="absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all duration-200"
							 class:left-0.5={!scrollBlurEnabled}
							 class:left-5={scrollBlurEnabled}></div>
					</div>
				</button>
				{/if}
			</div>
		...
		<div class="">
			...
			{/if}
			<!-- Banner Transparency Switch (仅横幅模式显示) -->
			{#if isBannerTransparencySwitchable && wallpaperMode === WALLPAPER_BANNER}
			<button
				class="w-full btn-regular rounded-md py-2 px-3 flex items-center gap-3 text-left active:scale-95 transition-all relative overflow-hidden"
				class:bg-(--btn-regular-bg-hover)={bannerTransparencyEnabled}
				onclick={toggleBannerTransparencyEnabled}
			>
				<Icon icon="material-symbols:overview-outline" class="text-[1.25rem] shrink-0"></Icon>
				<span class="text-sm flex-1">{i18n(I18nKey.bannerTransparency)}</span>
				<div class="w-10 h-5 rounded-full transition-all duration-200 relative"
					 class:bg-(--primary)={bannerTransparencyEnabled}
					 class:bg-(--btn-regular-bg-active)={!bannerTransparencyEnabled}>
					<div class="absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all duration-200"
						 class:left-0.5={!bannerTransparencyEnabled}
						 class:left-5={bannerTransparencyEnabled}></div>
				</div>
			</button>
			{/if}
